...
[ 'title' => 'Students', 'href' => url('/dashboard/orders'), 'icon' => 'shopping-cart', 'count' => 6 ],
];

// Top stats
$analytics = [
[ 'title' => 'Total Students', 'count' => 1234, 'icon' => 'users', 'unit' => '', 'detailLink' => url('/analytics/patients') ],
[ 'title' => 'Pending', 'count' => 156, 'icon' => 'stethoscope', 'unit' => '', 'detailLink' => url('/analytics/consultations') ],
[ 'title' => 'Approved', 'count' => 42, 'icon' => 'calendar', 'unit' => '', 'detailLink' => url('/analytics/appointments') ],
];
...
<div class="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
        ...
        <div class="flex h-full max-h-screen flex-col gap-2">
            <div class="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
                <a href="{{ url('/') }}" class="flex items-center gap-2 font-semibold text-gray-900 dark:text-gray-100">
                    ...
                    <span>MUBS INTERNSHIP PORTAL</span>
                </a>
                <button class="ml-auto h-8 w-8 inline-flex items-center justify-center rounded-md border bg-white dark:bg-gray-900" type="button">
    ...
    <div class="flex flex-col">
        ...
        <header class="flex h-14 items-center gap-4 border-b bg-gray-50 dark:bg-gray-800/40 px-4 lg:h-[60px] lg:px-6" x-data="{ open:false, userMenu:false }">
            ...
            <!-- Mobile Sheet -->
            <div x-show="open" x-transition.opacity class="fixed inset-0 z-40 bg-black/40 md:hidden" @click="open=false"></div>
            <aside x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="fixed z-50 top-0 left-0 h-full w-72 bg-white dark:bg-gray-900 shadow-md md:hidden p-6">
            ...
            <div class="w-full flex-1">
                <form onsubmit="return false;">
                    <div class="relative">
                        ...
                        <input type="search" placeholder="Search products..." class="w-full appearance-none bg-white dark:bg-gray-900 border rounded-md h-9 pl-8 pr-3 text-sm shadow-none md:w-2/3 lg:w-1/3 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    </div>
                </form>
            </div>
            ...
            <div class="relative" @click.outside="userMenu=false">
                ...
                <div x-show="userMenu" x-transition class="absolute right-0 mt-2 w-56 rounded-md border bg-white dark:bg-gray-900 shadow-lg overflow-hidden z-10">
                    <div class="px-3 py-2 text-center text-sm font-semibold">Jb web developer</div>
                    <div class="px-3 pb-2 text-center text-xs text-gray-500">jbwebdeveloper@gmail.com</div>
                    <div class="border-t my-1"></div>
                    <button class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">Settings</button>
                    <button class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">Support</button>
                    <div class="border-t my-1"></div>
                    <button class="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">Logout</button>
                </div>
            </div>
        </header>
        ...
        <div class="flex min-h-screen w-full flex-col">
            <div class="px-4 sm:px-6 lg:px-8 py-4" x-data="{ showCreate:false }">
                ...
                <div class="overflow-x-auto rounded border border-gray-300 shadow-sm">
                    <table class="min-w-full divide-y-2 divide-gray-200">
                        ...
                        <tr class="*:font-medium *:text-gray-900">
                            ...
                            <th class="px-3 py-2 whitespace-nowrap">Organisation</th>
                            <th class="px-3 py-2 whitespace-nowrap">Student Contact</th>
                            <th class="px-3 py-2 whitespace-nowrap">Notes (Opt)</th>
                        ...
                        <tr class="*:text-gray-900 hover:bg-gray-50 transition-colors *:first:font-medium">
                            ...
                            <td class="px-3 py-2 whitespace-nowrap">{{ $student->organization_name }}</td>
                            <td class="px-3 py-2 whitespace-nowrap">{{ $student->student_contact }}</td>
                            <td class="px-3 py-2 whitespace-nowrap">{{ $student->notes }}</td>
                            <td class="px-3 py-2 whitespace-nowrap text-right">
                                <div class="inline-flex items-center gap-2" x-data="{ open:false }">
                                    ...
                                    <div x-show="open" x-transition class="fixed z-50 inset-0 flex items-center justify-center p-4">
                                        <div class="w-full max-w-lg rounded-lg border bg-white shadow-lg" @click.stop>
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-3 py-6 text-center text-gray-500">No students found. Click "Add Student" to create one.</td>
                        </tr>
                        @endforelse
                ...
                <div x-show="showCreate" x-transition.opacity class="fixed inset-0 z-40 bg-black/40" @click="showCreate=false"></div>
                <div x-show="showCreate" x-transition class="fixed z-50 inset-0 flex items-center justify-center p-4">
                    <div class="w-full max-w-lg rounded-lg border bg-white shadow-lg" @click.stop>
                        <div class="flex items-center justify-between p-4 border-b">
